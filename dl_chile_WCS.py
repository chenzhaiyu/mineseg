...
def create_rectangle_kml(nw, se, filename="rectangle.kml"):
    """
...
# must be square
def get_tile_sizes(desired_tile_size=256, max_request_size=2500):
...
def process_chile(big_bbox, tile_collection_size):
    ...
    for corner, coords in zip(corners, corner_coords):
        utm_coords = convert_to_utm(coords[0], coords[1])
...
def get_urls(country_polygon, parameter):

    west, south, east, north = country_polygon.bounds

    tiles_per_dim, tile_collection_px_per_dim = get_tile_sizes(desired_tile_size=parameter["desired_tile_size"],
    ...
    create_rectangle_kml((west, north), (east, south))
    ...
    while pos_north > south:
        ...
        hpos = 0
        while pos_east > west:
            pos_south, pos_west = precise_new_location(pos_north, pos_east, dist_in_meter, dist_in_meter)
            ...
            if country_polygon.intersects(box(pos_west, pos_south, pos_east, pos_north)):
                ...
                # generate url
                url = ("https://sh.dataspace.copernicus.eu/ogc/wcs/15705528-7c35-4373-b499-d1b6b86015a6?"
                       "SERVICE=WCS&"
                       "VERSION=1.0.0&"
                       "REQUEST=GetCoverage&"
                       "FORMAT=GeoTIFF&"
...
def download_url(url, file_name_suffix="img", out_path="./out/"):
...
def precise_new_location(lat, lon, dist_to_west, dist_to_south):
...
def split_geotiff(input_file, output_folder, dim_size):
...
param = {}
param["client_id"] = "15705528-7c35-4373-b499-d1b6b86015a6"
param["desired_tile_size"] = 256
param["max_request_size"] = 2500
param["gsd"] = 10
param["maxcc"] = "15"
...
param["format"] = "image/tiff"
param["time_start"] = "2021-01-01"
param["time_end"] = "2022-12-31"


# get the URL's (must be less than 10000)
urls = get_urls(countryPolygon, param)

print("url count: ", len(urls))

# download each URL
for url in urls:
    f_name = download_url(url, country_name)
    dim_size, big_img_size = get_tile_sizes(param["desired_tile_size"], param["max_request_size"])
    split_geotiff(f_name, "./out/patches", dim_size)
